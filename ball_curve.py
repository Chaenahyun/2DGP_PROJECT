...
pitcher = load_image('pitcher_test.png')
...
def set_target_position(index, is_strike=True):
    global start_pitching, ball_x, ball_y, ball_size, bezier_t
    ...
    if not start_pitching:
        ...
        if is_strike:
            set_target(target_positions_strike[index])
        else:
            set_target(target_positions_ball[index])


def set_target(target):
    global target_x, target_y, bezier_points

    target_x, target_y = target
    # 초기 공에서 목표 지점까지의 베지어 곡선 좌표 계산
    bezier_points.clear()
    for t in range(0, 101):
        bezier_t = t / 100.0
        x = get_bezier_point(initial_ball_x, target_x, target_x, target_x, bezier_t)
        y = get_bezier_point(initial_ball_y, initial_ball_y, target_y, target_y, bezier_t)
...
def draw():
    ...
    pitcher.draw(pitcher_x, pitcher_y, 100, 100)
    if start_pitching and bezier_t <= 1.0:
        # 볼을 그릴 때 크기를 적용
        ball.draw(ball_x, ball_y, ball_size, ball_size)
        ball_size += 0.1
    update_canvas()

# 업데이트 함수에서 목표 지점에 도달하면 애니메이션 종료
def update():
    global start_pitching, ball_frame, ball_frame_count, ball_x, ball_y, target_x, target_y, speed, bezier_t

    if start_pitching and bezier_t <= 1.0:
        elapsed_time = get_time()

        if elapsed_time > ball_delay:
            ball_frame = (ball_frame + 1) % ball_frame_count

            # 베지어 곡선 좌표 계산
            index = int(bezier_t * 100)
            target_x, target_y = bezier_points[index]

            # 벡터 길이 계산
            dir_x = target_x - ball_x
            dir_y = target_y - ball_y

            # 벡터 길이가 0이 아닌 경우에만 정규화하고 속도 벡터 계산
